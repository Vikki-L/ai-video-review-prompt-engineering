...
```mermaid
flowchart TD
    A["① AI 给出审核结果<br/>decision: pass / review / reject"]
    B["② Coach 人工复核<br/>同意 AI → confirm<br/>不同意 → override（必填 reason）"]
    C["③ 标记 Badcase<br/>AI=pass, Coach=reject ❌<br/>AI=reject, Coach=pass ❌<br/>→ 自动进入 Badcase 库"]
    D["④ Badcase 分类聚类<br/>按出错维度聚类<br/>按视频类型聚类<br/>找重复出现的错误模式"]
    E["⑤ Prompt 优化<br/>典型 badcase 加进反例<br/>加强相关维度的判断标准<br/>调整 enum 边界<br/>发布新版 prompt"]
    F["🔄 下一轮审核用新版 prompt<br/>同类错误显著减少"]
